<template>
  <div class="mb-4" v-if="Field && Field.IsVisible">
    <slot name="label">
      <div class="relative h-8">
        <svg v-if="Field.LabelIcon !== ''" class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd" :d="Field.LabelIcon">
          </path>
        </svg>
        <label :for="Field.Id" v-if="Field.Label"
          class="absolute left-8 top-0 block text-sm font-medium text-gray-700 dark:text-gray-400"> {{ Field.Label }}
        </label>
      </div>
    </slot>
    <slot name="component">
      <label v-if="Field.IsReadonly">{{ Field.Value }}</label>
      <div v-else class="relative">
        <input :id="Field.Id" :name="Field.Id" type="input" @input="OnInput(Field)"
          :placeholder="Field.PlaceholderText" :class="Field.CssClass" v-model="Field.Value" />
        <span class="absolute right-3.5 top-1/2 -translate-y-1/2" v-if="HasValidationErrors">
          <svg v-if="Field.IsValid" width="16" height="16" viewBox="0 0 16 16" fill="none"
            xmlns="http://www.w3.org/2000/svg">
    ...
    <slot name="help">
      <span v-if="HasValidationErrors">
        <p :id="`${Field.Id}-help`" v-if="Field.HelpText" :class="[
          Field.IsValid ? 'mt-1.5 text-theme-xs text-success-500' : 'mt-1.5 text-theme-xs text-error-500',
        ]"> {{ Field.HelpText }}</p>
      </span>
    </slot>
  </div>
