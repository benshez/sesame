...
export class FormBuilder<TVisibility = null, TValidation = null, TScorer = null, TStep = null, TField = null> implements IBasePage<TVisibility, TValidation, TScorer, TStep, TField> {
  ...
  GetCurrentStepFields = (StepIndex: number = 0): Array<IField> => {
  ...
  GetField = (Id: string, StepIndex: number): IField => {
    const Fields: Array<IField> = this.GetCurrentStepFields(StepIndex);

    this.Field = Fields.find(Field => Field.Id === Id) as IField as TField;

    return this.Field as IField;
  }
