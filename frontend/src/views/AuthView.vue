...
const isSignIn = ref<boolean | false>(false);
...
const signIn = async (_: Event) => {
  ...
  if (response.status === "WRONG_CREDENTIALS_ERROR") {
    formStore.getElement("email").isValid = false;
    formStore.getElement("email").helpText = "Invalid credentials";
    formStore.getElement("password").isValid = false;
    formStore.getElement("password").helpText = "Invalid credentials";
    return;
  }
  ...
  if (response.status === "FIELD_ERROR") {
    response.formFields.forEach((item) => {
      if (item.id === "email") {
        formStore.getElement("email").isValid = false;
        formStore.getElement("email").helpText = item.error;
      } else if (item.id === "password") {
        formStore.getElement("password").isValid = false;
        formStore.getElement("password").helpText = item.error;
      }
    });
    return;
  }
