/*DROP TABLE Event_Type, Organization, Venue, Event, Attendee, Event_Tickets, Partner, Event_Partner,  Event_Ticket_Assignment, Country, Event_Status; 

DROP DATABASE IF EXISTS sesame;

CREATE DATABASE sesame
    WITH
    OWNER = sesame_supertokens_user
    ENCODING = 'UTF8'
...
(
	status_id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
	tenant_id character varying(64) COLLATE pg_catalog."default" NOT NULL DEFAULT 'public'::character varying,
	name VARCHAR(50) NOT NULL,
	active boolean NOT NULL
);
COMMENT ON TABLE Event_Status IS 'Table to store information about event status';

CREATE INDEX IF NOT EXISTS Event_Status_tenant_id_index
    ON public.Event_Status USING btree
    (tenant_id COLLATE pg_catalog."default" ASC NULLS LAST)
    WITH (fillfactor=100, deduplicate_items=True)
    TABLESPACE pg_default;
