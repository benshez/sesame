...
export class RoleService implements IRoleService {
  ...
  DeleteRole = async (request: IRoleRequest): Promise<{ status: "OK"; didRoleExist: boolean; }> => {
    const response = await UserRoles.deleteRole(request.role?.roleId as string);

    return response;
  }
  CreateNewRoleOrAddPermissions = async (request: IRoleRequest): Promise<{ status: "OK"; createdNewRole: boolean; }> => {
    const response = await UserRoles.createNewRoleOrAddPermissions(request.role?.roleId as string, request.role?.permissions as Array<string>);

    return response;
  }
  RemovePermissionsFromRole = async (request: IRoleRequest): Promise<{ status: "OK" | "UNKNOWN_ROLE_ERROR"; }> => {
    const response = await UserRoles.removePermissionsFromRole(request.role?.roleId as string, request.role?.permissions as Array<string>);

    return response;
  }
  GetPermissionsForRole = async (request: IRoleRequest): Promise<{ status: "OK"; permissions: string[]; } | { status: "UNKNOWN_ROLE_ERROR"; }> => {
    const response = await UserRoles.getPermissionsForRole(request.role?.roleId as string);

    return response;
  }
  GetRolesThatHavePermission = async (request: IRoleRequest): Promise<{ status: "OK"; roles: string[]; }> => {
    const response = await UserRoles.getRolesThatHavePermission(request.role?.permission as string);

    return response;
  }
  AddRoleToUser = async (request: IRoleRequest): Promise<{ status: "OK"; didUserAlreadyHaveRole: boolean; } | { status: "UNKNOWN_ROLE_ERROR"; }> => {
    const response = await UserRoles.addRoleToUser(request.baseRequest.tenantId, request.baseRequest.userId, request.role?.roleId as string);

    return response;
  }
  RemoveUserRole = async (request: IRoleRequest): Promise<{ status: "OK"; didUserHaveRole: boolean; } | { status: "UNKNOWN_ROLE_ERROR"; }> => {
    const response = await UserRoles.removeUserRole(request.baseRequest.tenantId, request.baseRequest.userId, request.role?.roleId as string);
